...
def get_fboss_docker_path(fboss_dir:str ):
    fboss_path = os.path.dirname(fboss_dir)
    docker_dir_path = os.path.join(fboss_path, "/fboss/oss/docker")
    print(f"fboss_dir={fboss_dir}")
    print(f"fboss_path={fboss_path}")
    print(f"docker_dir_path={docker_dir_path}")
    return docker_dir_path
...
def run_fboss_build(
    ...
    cmd_args.append(f"{fboss_path}:{CONTAINER_WORKDIR}:z")
